...
export default function AlertsPage() {
  ...
  const [form, setForm] = useState({
    symbol: '', alertType: 'PRICE_ABOVE', targetPrice: '', targetPercent: '', message: ''
  })
  ...
  const handleCreate = async e => {
    ...
    try {
      ...
      setShowForm(false)
      setForm({ symbol: '', alertType: 'PRICE_ABOVE', targetPrice: '', targetPercent: '', message: '' })
      loadAlerts()
    } catch {}
  }
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {/* Create form */}
      {showForm && (
        <div className="card border-violet-600/30">
          <h3 className="font-semibold text-white mb-4">Create Alert</h3>
          <form onSubmit={handleCreate} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select className="input" value={form.alertType}
            ...
            {(form.alertType === 'PRICE_ABOVE' || form.alertType === 'PRICE_BELOW') && (
              <div>
                ...
                <input type="number" step="0.01" className="input" placeholder="150.00"
            ...
            {form.alertType === 'PERCENT_CHANGE' && (
              <div>
                ...
                <input type="number" step="0.1" className="input" placeholder="5.0"
                  value={form.targetPercent} onChange={e => setForm({ ...form, targetPercent: e.target.value })} />
              </div>
            )}
            <div className="sm:col-span-2">
              <label className="block text-xs text-slate-400 mb-1">Custom Message (optional)</label>
              <input className="input" placeholder="Alert message..."
            ...
            <div key={alert.id} className={clsx(
              ...
              <div className="flex items-center gap-3">
                {alert.triggered
                  ...
                  : <Bell size={20} className={clsx('flex-shrink-0', alert.active ? 'text-violet-400' : 'text-slate-500')} />
                }
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {alert.triggered && <span className="text-xs bg-emerald-500/20 text-emerald-400 px-2 py-0.5 rounded">TRIGGERED</span>}
                  </div>
              ...
              <div className="flex items-center gap-2">
                {!alert.triggered && (
                  <button onClick={() => handleToggle(alert.id)} className="text-slate-400 hover:text-violet-400 transition-colors">
                    {alert.active ? <ToggleRight size={20} className="text-violet-400" /> : <ToggleLeft size={20} />}
                  </button>
                )}
